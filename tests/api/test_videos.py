...
def mock_db():
    """Mock database."""
    with patch('api.routes.videos.get_db') as mock:
        db = MagicMock()
...
def mock_storage():
    """Mock storage."""
    with patch('api.routes.videos.get_storage') as mock:
        storage = MagicMock()
...
class TestListVideos:
    ...
    def test_list_videos_success(self, client, mock_db, sample_video_metadata):
    ...
    def test_list_videos_empty(self, client, mock_db):
    ...
    def test_list_videos_with_status_filter(self, client, mock_db, sample_video_metadata):
...
class TestGetVideo:
    ...
    def test_get_video_success(self, client, mock_db, sample_video_metadata):
        """Test successful video retrieval."""
        mock_db.get_video.return_value = sample_video_metadata

        response = client.get("/api/videos/test-video-123")
...
class TestSignedUrl:
    """Tests for POST /api/videos/signed-url endpoint."""
...
class TestCreateVideo:
    ...
    def test_create_video_success(self, client, mock_db):
...
class TestProcessVideo:
    """Tests for POST /api/videos/{video_id}/process endpoint."""

    def test_process_video_success(self, client, mock_db, sample_video_metadata):
        ...
        response = client.post(
            "/api/videos/test-video-123/process",
            json={
                "compress": True,
                "chunk": True,
                "extract_audio": True
    ...
    def test_process_video_not_found(self, client, mock_db):
        """Test processing for non-existent video."""
        mock_db.get_video.return_value = None

        response = client.post(
            "/api/videos/nonexistent/process",
            json={
                "compress": True,
                "chunk": True,
                "extract_audio": True
...
class TestDeleteVideo:
    ...
    def test_delete_video_success(self, client, mock_db, mock_storage, sample_video_metadata):
        ...
        response = client.delete("/api/videos/test-video-123")

        assert response.status_code == 204
        mock_storage.delete_file.assert_called()

    def test_delete_video_not_found(self, client, mock_db):
...
class TestGetResults:
    """Tests for GET /api/videos/{video_id}/results endpoint."""
    ...
    def test_get_results_success(self, client, mock_db):
        ...
        response = client.get("/api/videos/test-video-123/results")
    ...
    def test_get_results_with_type_filter(self, client, mock_db):
        ...
        response = client.get("/api/videos/test-video-123/results?result_type=scene_analysis")
