...
def _make_csv() -> bytes:
    ...
    scores = {0: 5, 10: 5, 20: 4, 30: 1, 40: 4, 50: 5, 60: 5, 70: 6, 80: 7,
              90: 8, 100: 9, 110: 8, 120: 7, 130: 6, 140: 5, 150: 5, 160: 5,
              170: 4, 180: 5, 190: 5, 200: 5}
...
def test_process_engagement_job_happy_path(worker):
    ...
    engagement.explain.return_value = {
        "peaks": [
            {"rank": 1, "timestamp_sec": 100.0, "scene_summary": "fight peak",
             "explanation": "punch lands", "key_actors": ["hero"], "key_events": ["punch"], "key_objects": []},
        ...
        "valleys": [
            {"rank": 1, "timestamp_sec": 30.0, "scene_summary": "lull",
             "explanation": "exposition", "key_actors": [], "key_events": [], "key_objects": []},
    ...
    # 3. Timeseries was uploaded as JSON
    assert storage.upload_bytes.called
    upload_args = storage.upload_bytes.call_args
    assert upload_args.kwargs.get("content_type") == "application/json"
    ...
    assert set(call_kwargs["contexts"].keys()) == {
        p.timestamp_sec for p in call_kwargs["peaks"]
...
def test_process_pending_engagement_marks_failed_on_error(worker):
    ...
    db.get_pending_engagement_jobs.return_value = [
        {"job_id": "eng-1", "video_id": "v1", "source_scene_job_id": "src-1",
         "barc_gcs_path": "gs://x/y.csv", "config": None}
    ]
    ...
    failed_calls = [
        c for c in db.update_engagement_job_status.call_args_list
        if EngagementJobStatus.FAILED in c.args
    ]
