// Lifecycle for the avatar live session: opens the WS, wires up the audio +
// video sinks, and tears everything down on unmount or when the caller
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      try {
        ...
        await avatarsApi.liveConfig(avatarId)
        if (cancelled) return
