...
- [ ] `libs/basketball/jersey.py`: collect `number`-class crops per player track (detector from Epic 2 story 2).
- [ ] Legibility gate: discard blurred/tiny crops (size + Laplacian-variance threshold) before classification.
- [ ] Digit recognition: start with RapidOCR digit mode on upscaled crops; if accuracy is insufficient, train the small two-digit classifier (ResNet-32-style, 0–99) on Roboflow number crops — decision recorded in the story PR.
- [ ] Tracklet vote: number accepted after ≥ 3 consistent reads (confidence scales with vote margin); below threshold → `jersey: null`.
- [ ] Attach voted jersey to the shooter track on each shot event; roster map (jersey → player name) as optional CLI input `--roster roster.yaml`.
